...
import { type ConsoleStatus, ConsoleType, type SonyConsole } from "../models/Core";
import { DiscoverCommand, type DiscoverPacket, type Discovery, type LocalAddr } from "../models/Discovery";
import { createSocket, type RemoteInfo, type Socket } from "dgram";
import bindings from "bindings";
import { type Chiaki, type Wakeup } from "../models/Chiaki";
...
const discoveryTimeout: number = 3000;
...
export function wakeup (host: SonyConsole): boolean {
	const wakeup: Wakeup = new chiaki.Wakeup();
	if (host.registKey && host.address) {
		return wakeup.wake(host.registKey, host.address, host.hostType == ConsoleType.PS5);
	}
	return false;
}
...
function attemptBind (discovery: Discovery, port: number, protocol_version: string, callbackPort: number, callback: DiscoveryCallback, startingCallback: DiscoveryStartCallback) {
	...
	discovery.socket?.bind(port, undefined, () => {
		...
		if (discoverPacket !== null) {
			...
			setInterval(() => { sendDiscoveryPacket(startingCallback, discovery.socket, packetUint8Array, callbackPort); }, discoveryTimeout);
		} else {
